<script lang="ts">
  ...
  onMount(() => {
    ...
    listProcessNames().then(processes => {
      ...
      for (const process of processes) {
        ...
        if (cleanName.startsWith('windows') ||
  ...
  function updateFilteredList() {
    ...
    if (searchTerm) {
      combinedList = excluded.filter(p => 
  ...
  function isExcluded(processName: string): boolean {
    return cfg?.process_exclusion_list.some(p => 
  ...
  async function remove(name: string, e?: MouseEvent) {
    ...
    if (!cfg) return;
    const next = cfg.process_exclusion_list.filter((x) => 
    ...
    <div class="row">
      <div class="dropdown-container">
        <input 
          ...
          on:focus={() => { showDropdown = true; updateFilteredList(); }}
