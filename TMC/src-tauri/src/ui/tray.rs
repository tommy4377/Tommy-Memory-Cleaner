...
pub fn create_tray_icon(percentage: u8, bg_hex: &str, text_hex: &str, transparent: bool) -> Image<'static> {
    ...
    let mut img: RgbaImage = if transparent {
        ImageBuffer::from_fn(render_size, render_size, |_, _| {
            Rgba([0, 0, 0, 0])
        })
    } else {
        ImageBuffer::from_fn(render_size, render_size, |_, _| {
...
fn get_default_icon() -> Image<'static> {
    DEFAULT_ICON.get_or_init(|| {
        load_default_icon().unwrap_or_else(|e| {
            ...
            Image::new_owned(vec![0u8; (ICON_SIZE * ICON_SIZE * 4) as usize], ICON_SIZE, ICON_SIZE)
        })
    }).clone()
}
...
pub fn refresh_tray_icon(app: &AppHandle) {
    let (show_mem, mem_percent) = {
        ...
        let show = state.cfg.lock().map(|c| c.tray.show_mem_usage).unwrap_or(true);
        (show, 0u8) 
    };
