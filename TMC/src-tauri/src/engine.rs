...
use crate::logging::event_viewer::{log_optimization_event, log_error_event};
use crate::os;
...
impl Engine {
    ...
    pub fn optimize<F>(&self, reason: Reason, areas: Areas, mut progress: Option<F>) -> anyhow::Result<OptimizeResult>
where F: FnMut(u8, u8, String)
{
    // FIX: Pre-acquisisci tutti i privilegi necessari PRIMA di iniziare
    tracing::info!("Starting optimization with reason: {:?}, areas: {:?}", reason, areas);
    ...
    for priv_name in &required_privs {
        ...
        for attempt in 1..=3 {
            match crate::memory::privileges::ensure_privilege(priv_name) {
                ...
                Err(e) => {
                    if attempt < 3 {
                        tracing::warn!("Failed to acquire {} (attempt {}): {}, retrying...", 
                                      priv_name, attempt, e);
                        std::thread::sleep(std::time::Duration::from_millis(100 * attempt as u64));
                    } else {
                        let error_msg = format!("Failed to acquire privilege {} after 3 attempts: {}", priv_name, e);
    ...
    tracing::info!("Acquired {}/{} required privileges", acquired_privs, required_privs.len());
    ...
    if validated_areas != areas {
        ...
        tracing::warn!("Some memory areas are not available on this Windows version: {:?}", missing);
    }
        ...
        let res = match rx.recv_timeout(OPERATION_TIMEOUT) {
            Ok(result) => {
                ...
                if let Err(e) = handle.join() {
                    tracing::warn!("Thread panicked during operation {}: {:?}", display_name, e);
                }
                result
            }
            Err(mpsc::RecvTimeoutError::Timeout) => {
                tracing::warn!("Operation {} timed out after {:?}", display_name, OPERATION_TIMEOUT);
            ...
            Err(mpsc::RecvTimeoutError::Disconnected) => {
                ...
                if let Err(e) = handle.join() {
                    tracing::warn!("Thread panicked during operation {} (disconnected): {:?}", display_name, e);
                }
    ...
    loop {
        ...
        if freed.abs() >= 1024 * 1024 || retry_count >= MAX_RETRIES {
            if retry_count > 0 {
                tracing::info!("Memory measurement stabilized after {} retries", retry_count);
            }
            break;
        }
        
        retry_count += 1;
        tracing::debug!("Memory change too small ({} bytes), retrying measurement (attempt {})", 
                       freed, retry_count);
    ...
    if after.physical.free.bytes > MAX_SAFE_BYTES || before.physical.free.bytes > MAX_SAFE_BYTES {
        tracing::warn!("Memory values exceed safe range ({} bytes), clamping for calculation", MAX_SAFE_BYTES);
    }
    
    // Cast sicuro dopo il clamping
    let freed_phys = (after_phys_safe as i64)
        .saturating_sub(before_phys_safe as i64);
    let freed_commit = (after_commit_safe as i64)
        .saturating_sub(before_commit_safe as i64);
    ...
    if freed_phys.abs() > 1024 * 1024 || has_successful_area {
        ...
        let profile_name = self.cfg.lock()
